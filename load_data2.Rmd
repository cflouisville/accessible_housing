...
dis_by_age <- age_all_5 %>%
  filter(year == 2023, sex == "total", race == "total", var_type == "percent",!is.na(age_group_5_yrs)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any),
               names_to = "Disability_Type", 
               values_to = "Percent") %>% 
  mutate(Disability_Type = recode(Disability_Type, dis_physical = "Physical",
...
plot_overall_time <- ggplot(dis_prev_by_type_over_time, aes(x = year, y = Percent, color = Disability_Type, group = Disability_Type)) +
  geom_segment(
    data = tibble(x = seq(min(dis_prev_by_type_over_time$year), 2023, by = 1),
                  y1 = 0,
  ...
  data = tibble(x = seq(min(dis_prev_by_type_over_time$year), 2023, by = 2),
                  y1 = 0,
                  y2 = max(dis_prev_by_type_over_time$Percent)),
    aes(x = x, xend = x, y = y1, yend = y2),
    inherit.aes = FALSE,
