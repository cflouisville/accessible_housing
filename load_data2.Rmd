...
age_excel <- age_all %>%
  filter(year == 2024, sex == "total", race == "total", var_type %in% c("percent", "estimate"), !is.na(age_group_4)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Category", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  mutate(Category = recode(Category, dis_physical = "Physical",
                                  ...
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  rename(Age = age_group_4, Number = estimate, Percent = percent) %>%
...
# over 65 vs under
age_new <- age_all %>%
   filter(year == 2024, sex == "total", race == "total", var_type %in% c("percent", "estimate", "population"), !is.na(age_group_4)) %>%
  
  mutate(Age = case_when(
      age_group_4 %in% c("50-64", "Under 50") ~ "Under 65",
      age_group_4 %in% c("80 or Older", "65-80") ~ "Over 65",
      TRUE ~ NA_character_
    )) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
...
#clean table
dis_by_age <- age_all %>%
  filter(year == 2024, sex == "total", race == "total", var_type == "percent",!is.na(age_group_4)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>% 
  mutate(Disability_Type = recode(Disability_Type, dis_physical = "Physical",
