---
title: "Accessible Housing"
output:
  html_document:
    css: styles.css
    accent: "#7bdcb5"
    self_contained: false
    toc: yes
    toc_float: yes
    code_folding: hide
date: "2026-01-21"
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), 'docs/index.html')) })
---

```{r setup, echo=FALSE, message=FALSE, warning=FALSE}
#install.packages('showtext', dependencies = TRUE) #for montserrat font in ggplot

library(showtext)
library(glptools)
library(openxlsx)
glp_load_packages(T)

font_add_google("Montserrat", "Montserrat")
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, cache = FALSE,  
                      dev.args=list(bg="transparent"), fig.width=9, fig.height=6) 

```

```{r read_data, eval = FALSE}
# Analyze ACS disability data
# Read in data

# @Morgan this file is so named because it is ACS Microdata that is cut down to just counties/FIPS (as opposed to observations across the MSA, which takes longer to read in and work with) and includes replicate weights.

options(arrow.skip_nul = TRUE)

load("data/acs_micro_subset_2024.RData")

# acs_micro <- arrow::read_feather("C:/Users/harri/OneDrive/GLP/glpdata/data-raw/microdata/acs_micro_FIPS_repwts.feather")

# disability_data <- acs_micro %>%
#   filter(year >= 2008, FIPS == "21111")
# 
# save(disability_data, file = "interim_data/acs_micro_subset.RData")

# acs_micro <- arrow::read_feather("C:/Users/morga/OneDrive - The Community Foundation of Louisville, Inc/acs_micro_FIPS_repwts.feather") |> 
#   as.data.frame() # had to add this line bc it wasn't reading the feather file anymore??

# base_example <- acs_micro[acs_micro$FIPS == "21111",]
# 
# dplyr_example <- filter(acs_micro, FIPS == "21111")
# 
# dplyr_example2 <- acs_micro %>%
#   filter(FIPS == "21111")
# 
# acs_micro %<>% #modify in place
#   filter(FIPS == "21111")
# 
# dploy_example3 <- acs_micro %>%
#   select(FIPS, race)
# 
# dplyr_example4 <- acs_micro %>%
#   select(FIPS, race) %>%   # Chopping down to columns!
#   filter(FIPS == "21111")
# 
# 
# disability_data[1:100,] %>% select(dis_physical)

# disability_data <- acs_micro %>%
#  filter(year >= 2008, FIPS == "21111")

```

```{r clean_data, eval = FALSE}
# Clean data, set grouping variables

# The ACS disability questions were significantly modified in 2008, so we will only use data from 2008 onward so that all of the years are comparable. Limit scope to Lou to make code faster initially- may undo subset later

#Morgan- load("C:/Users/morga/OneDrive - The Community Foundation of Louisville, Inc/acs_micro_subset.RData")

# RECODE DISABILITY VARIABLES. Find info here: https://usa.ipums.org/usa/index.shtml
# @Morgan case_when evaluates from top to bottom. If the value on the left is true, it assigns the value on the right.

disability_data %<>%
  mutate(
    # Ambulatory difficulty
    dis_physical = case_when(
      DIFFPHYS == 0 ~ NA, #tilde indicates formula
      DIFFPHYS == 1 ~ F,
      DIFFPHYS == 2 ~ T),
    
    # Independent living difficulty
    dis_independent = case_when(
      DIFFMOB == 0 ~ NA,
      DIFFMOB == 1 ~ F,
      DIFFMOB == 2 ~ T), 
    
    # Self-care difficulty
    dis_selfcare  = case_when(
      DIFFCARE  == 0 ~ NA,
      DIFFCARE  == 1 ~ F,
      DIFFCARE  == 2 ~ T), 
    
    # Vision difficulty
    dis_vision = case_when(
      DIFFEYE == 0 ~ NA,
      DIFFEYE == 1 ~ F,
      DIFFEYE == 2 ~ T), 
    
    # Hearing difficulty
    dis_hearing  = case_when(
      DIFFHEAR  == 0 ~ NA,
      DIFFHEAR  == 1 ~ F,
      DIFFHEAR  == 2 ~ T),
    
    dis_cognitive = case_when(
      DIFFREM == 0 ~ NA, 
      DIFFREM == 1 ~ F, 
      DIFFREM == 2 ~ T),
    
    dis_sensory = case_when(
      DIFFSENS ==0 ~ NA, 
      DIFFSENS == 1 ~ F,
      DIFFSENS == 2 ~T),
    
    # Any disabling condition
    # @Morgan switching up the syntax for funsies :)
    # R is a "vectorized" programming language. One of the most poweful aspects of R, and the hardest to wrap your head around, is when things are happening within individual row elements vs. across entire column vectors. For example, the plus operator here works within individual rows. If you were to use the sum function instead, it would add up all of the values within the columns. If I did that, I would find that dis_any would be T for every observation because sum(those variables) would evaluate to like 300000 for every single row.
    dis_any = if_else((dis_physical + dis_independent + dis_selfcare + dis_vision + dis_hearing + dis_cognitive) > 0, T, F),
    
    dis_any_4 = if_else((dis_physical + dis_selfcare + dis_vision + dis_hearing) > 0, T, F),

    
    # RECODE OTHER CATEGORY VARIABLES
    # Age group- Not sure how many buckets I want, so I make two versions! We will see what provides meaningful information, and how deep we can disaggregate before the data becomes to noisy.
    
    age_group_3 = case_when(      
      age %in% 5:18 ~ "5_18",
      age %in% 19:64 ~ "19_64",
      age > 65 ~ "65+"),
    
    #age group found in Harvard Survey Analysis -M
    age_group_4 = case_when( 
      age < 50 ~ "Under 50", 
      age %in% 50:64 ~ "50-64", 
      age %in% 65:80 ~ "65-80", 
      age > 80 ~ "80 or Older"), 
    
    age_group_9 = case_when(
      age %in% 0:9 ~ NA_character_, #@Morgan NA can have different types! NA alone is a logical. Common source of errors.
      age %in% 10:19 ~ "10_19", 
      age %in% 20:29 ~ "20_29", 
      age %in% 30:39 ~ "30_39",  
      age %in% 40:49 ~ "40_49",  
      age %in% 50:59 ~ "50_59",  
      age %in% 60:69 ~ "60_69", 
      age %in% 70:79 ~ "70_79", 
      age >= 80 ~ "80+"),
    
    age_group_5_yrs = case_when(
      age %in% 50:54 ~ "50-54",
      age %in% 55:59 ~ "55-59",
      age %in% 60:64 ~ "60-64",
      age %in% 65:69 ~ "65-69",
      age %in% 70:74 ~ "70-74",
      age %in% 75:79 ~ "75-79",
      age %in% 80:84 ~ "80-84",
      age %in% 85:90 ~ "85-90",
      age > 90 ~ "90 plus"),
    
    age_group_2 = case_when(
      age < 65 ~ "Under 65", 
      age >= 65 ~ "Over 65"
    ), 
    
  # Income group
  # @Morgan the relevant income in this situation is probably household income over individual earnings. Unlike other variables included here, this is the same for all members of a household.
  #^ so do we use income in our individual demographic breakdown charts at all? -M
  
    HHINCOME = replace(HHINCOME, HHINCOME == 99999, NA),
  
    income_group = case_when(
      HHINCOME < 20000 ~ "Under 20k",
      HHINCOME >= 20000 & HHINCOME < 50000 ~ "20k–49.9k",
      HHINCOME >= 50000 & HHINCOME < 75000 ~ "50k–74.9k",
      HHINCOME >= 75000 & HHINCOME < 100000 ~ "75k–99.9k",
      HHINCOME >= 100000 & HHINCOME < 150000 ~ "100k–149.9k",
      HHINCOME >= 150000 ~ "150k+",
      TRUE ~ NA_character_  ),  # catch-all for missing/unspecified values, "TRUE" could also be "is.na(HHICOME)"
  
  # Homeownership- Not sure if we'll use them all. Rent vs. own for sure, unsure about others.
  
    OWNCOST  = replace(OWNCOST, OWNCOST == 99999, NA),
    OWNERSHP = replace(OWNERSHP, OWNERSHP == 0, NA),
    RENTGRS  = replace(RENTGRS, RENTGRS == 0 & OWNERSHP == 1, NA),
    
    poverty = case_when( 
      POVERTY == -6 ~ NA, 
      POVERTY < 100 ~ TRUE, 
      POVERTY >= 100 ~ FALSE),

    homeownership = if_else(OWNERSHP == 1, T, F),

    hcost = if_else(homeownership == 1, OWNCOST, RENTGRS),
    cost_burden = if_else(hcost * 12 / HHINCOME > 0.3, T, F),
    severe_cost_burden = if_else(hcost * 12 / HHINCOME > 0.5, 1, 0),

    hh_type = case_when(
      homeownership  & !cost_burden ~ "noncb_homeowner",
      homeownership  & cost_burden  ~ "cb_homeowner",
      !homeownership & !cost_burden ~ "noncb_renter",
      !homeownership & cost_burden  ~ "cb_renter",
      TRUE ~ NA_character_)
    )

# MB Checking the disability variable recode. The colon selects several columns. Nifty!
# disability_data[1:100,] %>% select(dis_physical:dis_any)


## HOUSEHOLD
# @Morgan this makes a variable that flags whether anyone in the household (HH) has a disability!!

disability_data %<>%
  
  #@MB HHs are identified by unique values of SERIAL. (SERIAL is repeated across years, hence the need to group by year as well)
  group_by(year, SERIAL) %>% 
  
  #@MB back to vectorized! The verb any is looking up and down the small subset of observations we have created for each HH via group_by to see if dis_physical is true in any of the rows. This type of operation usually takes way longer than simple row-wise operations, so this code is suuuuuper slow.
  
  mutate(
    any_dis_physical    = if_else(any(dis_physical), T, F), 
    any_dis_independent = if_else(any(dis_independent), T, F),
    any_dis_selfcare    = if_else(any(dis_selfcare), T, F),
    any_dis_vision      = if_else(any(dis_vision), T, F),
    any_dis_hearing     = if_else(any(dis_hearing), T, F),
    any_dis_cognitive   = if_else(any(dis_cognitive), T, F), 
    any_dis_sensory     = if_else(any(dis_sensory), T, F), 
    any_dis_any         = if_else(any(dis_any), T, F), #@MB lol at this naming convention :)
    any_dis_any_4       = if_else(any(dis_any_4), T, F)) %>%

  ungroup() 

#@MB saving 
arrow::write_feather(disability_data, "interim_data/disability_data.feather")

```

```{r std_age}
std_age <- read_csv("data/singleages.pops.tables.csv", skip = 1)

std_age <- std_age[1:91,]

std_age %<>%
  transmute(
    age = as.numeric(str_sub(`...1`, 1, 2)),
    population = `Single Ages to 89...2`,
    age_group_4 = case_when( 
      age < 50 ~ "Under 50", 
      age %in% 50:64 ~ "50-64", 
      age %in% 65:80 ~ "65-80", 
      age > 80 ~ "80 or Older")) %>%
  group_by(age_group_4) %>%
  reframe(population = sum(population))

std_age %<>%
  mutate(weight = population / sum(population)) %>%
  select(-population)

save(std_age, file = "data/std_age.RData")

```

```{r initial_calcs_age_adjusted, eval = FALSE}
# ## Process data
# 
# This takes the data that was read, filtered, processed, and saved above and does calculations.

disability_data <- arrow::read_feather("interim_data/disability_data.feather")

# What percentage of Louisvillians have a disability?

# @Morgan, the GLP code does have fairly crappy documentation that you can access in R! e.g. ?survey_by_demog
# But the documentation is at times outdated or incomplete. You can always view inside the function, particularly to see what arguments are provided.


# All disabilities x race and sex
individual_dis_physical    <- survey_by_demog(disability_data, "dis_physical",    "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_independent <- survey_by_demog(disability_data, "dis_independent", "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_selfcare    <- survey_by_demog(disability_data, "dis_selfcare",    "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_vision      <- survey_by_demog(disability_data, "dis_vision",      "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_hearing     <- survey_by_demog(disability_data, "dis_hearing",     "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_cognitive   <- survey_by_demog(disability_data, "dis_cognitive",   "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_sensory   <- survey_by_demog(disability_data, "dis_sensory",   "PERWT", 
                                              other_grouping_vars = "age_group_4")
individual_dis_any         <- survey_by_demog(disability_data, "dis_any",         "PERWT", 
                                              other_grouping_vars = "age_group_4")


# Combine data frames
# @MB: the functions for you to know are left_join and full_join, etc.
# This would work here because the data frame starts out super organized and the columns in common are tidy, but normally you need to do more specification on which columns are supposed to match up, etc.
# individual_dis <- left_join(individual_dis_physical, individual_dis_independent)
# individual_dis <- left_join(individual_dis, individual_dis_selfcare)
# ...

# Custom glptools shortcut
individual_dis <- bind_df(individual_dis_physical, 
                          individual_dis_independent,
                          individual_dis_selfcare,
                          individual_dis_vision,
                          individual_dis_hearing,
                          individual_dis_cognitive, 
                          individual_dis_any,
                          individual_dis_sensory,
                          by = c("FIPS", "year", "sex", "race", "var_type", "age_group_4"))

# Clean up the environment
rm(individual_dis_physical, individual_dis_independent, individual_dis_selfcare, individual_dis_vision,individual_dis_sensory, individual_dis_hearing, individual_dis_cognitive, individual_dis_any)

dis_vars <- c("dis_physical", "dis_independent", "dis_selfcare", "dis_vision", "dis_hearing", "dis_cognitive", "dis_sensory")


# Pivot table to create estimate and population columns
df <- individual_dis %>%
  filter(sex == "total", var_type %in% c("estimate", "population")) %>%
  select(-FIPS, -sex) %>%
  pivot_longer(dis_physical:dis_sensory) %>%
  pivot_wider(names_from = var_type, values_from = value)

df %<>%
  group_by_at(df %cols_in% c("age_group_4", "race", "name", "year")) %>%
  summarise(
    estimate = sum(estimate, na.rm = TRUE),
    population = sum(population, na.rm = TRUE)) %>%
  ungroup()

df <- df %>%
  left_join(std_age, by = "age_group_4") %>%
  group_by(race, name, year) %>%
  summarise(rate = sum(estimate / population * weight)*100) %>%
  ungroup()

# @MB turns out I DID write this code to let us group by other variables! Lit.
# Physical disability X age, income, and homeownership X race and sex

individual_dis_age_3     <- survey_by_demog(disability_data, 
                                        "dis_physical", 
                                        "PERWT", 
                                        other_grouping_vars = "age_group_3")

individual_dis_any_age_3 <- survey_by_demog(disability_data, 
                                        "dis_any", 
                                        "PERWT", 
                                        other_grouping_vars = "age_group_3")

individual_dis_age_4     <- survey_by_demog(disability_data, 
                                        "dis_physical", 
                                        "PERWT", 
                                        other_grouping_vars = "age_group_4")

individual_dis_age_9     <- survey_by_demog(disability_data, 
                                        "dis_physical", 
                                        "PERWT", 
                                        other_grouping_vars = "age_group_9")

individual_dis_any_age_4  <- survey_by_demog(disability_data, 
                                            "dis_any_4", 
                                            "PERWT", 
                                            other_grouping_vars = "age_group_4") 

individual_dis_income     <- survey_by_demog(disability_data, 
                                         "dis_physical", 
                                         "PERWT", 
                                         other_grouping_vars = "income_group")


individual_dis_homeownership <- survey_by_demog(disability_data, 
                                                "dis_physical", 
                                                "PERWT", 
                                                other_grouping_vars = "homeownership")

older_65   <- survey_by_demog(disability_data, "age_group_3",    "PERWT")
save(older_65, file = "interim_data/over_65.RData")
save(df, file = "interim_data/age_adjusted.rData")

dis_vars <- c("dis_physical", "dis_independent", "dis_selfcare", "dis_vision", "dis_hearing", "dis_cognitive")


income_and_hearing <- survey_by_demog(disability_data, 
                                         "dis_hearing", 
                                         "PERWT", 
                                         other_grouping_vars = "income_group")

```
                                        
```{r hh_process_data, eval=FALSE}
# What percentage of Louisville households include a person with a disability?

# @MB Every person within a household is identified by PERNUM. Variables that apply to the household will have the same value for every individual in that household, so we just need one row per household. We filter to just the first person in each household with PERNUM == 1.

disability_data_hh <- disability_data %>%
  filter(PERNUM == 1)

# Individual-level demographics don't apply at the household level, i.e. race and gender. I use breakdowns == "total" to stop creating numbers by race and gender. But income and homeownership still do apply.

# All disabilities

household_dis_physical    <- survey_by_demog(disability_data_hh, "any_dis_physical",    "HHWT", breakdowns = "total") 
household_dis_independent <- survey_by_demog(disability_data_hh, "any_dis_independent", "HHWT", breakdowns = "total")
household_dis_selfcare    <- survey_by_demog(disability_data_hh, "any_dis_selfcare",    "HHWT", breakdowns = "total") 
household_dis_vision      <- survey_by_demog(disability_data_hh, "any_dis_vision",      "HHWT", breakdowns = "total") 
household_dis_hearing     <- survey_by_demog(disability_data_hh, "any_dis_hearing",     "HHWT", breakdowns = "total") 
household_dis_cognitive   <- survey_by_demog(disability_data_hh, "any_dis_cognitive",   "HHWT", breakdowns = "total")
household_dis_sensory     <- survey_by_demog(disability_data_hh, "any_dis_sensory",     "HHWT", breakdowns = "total")
household_dis_any         <- survey_by_demog(disability_data_hh, "any_dis_any",         "HHWT", breakdowns = "total") 
household_dis_any_4       <- survey_by_demog(disability_data_hh, "any_dis_any_4", "HHWT", 
                                             breakdowns = "total")

household_dis <- bind_df(household_dis_physical, 
                         household_dis_independent,
                         household_dis_selfcare,
                         household_dis_vision,
                         household_dis_hearing,
                         household_dis_cognitive,
                         household_dis_sensory,
                         household_dis_any_4,
                         household_dis_any)

# Clean up the environment
rm(household_dis_physical, household_dis_independent, household_dis_selfcare, household_dis_vision,
   household_dis_hearing, household_dis_sensory, household_dis_cognitive, household_dis_any)

#@MB note HHWT rather than PERWT 

# Physical disability x income and homeownership
household_dis_income <- survey_by_demog(disability_data_hh, 
                                         "any_dis_physical", 
                                         "HHWT", 
                                         other_grouping_vars = "income_group",
                                         breakdowns = "total")

household_dis_homeownership <- survey_by_demog(disability_data_hh, 
                                               "any_dis_physical", 
                                               "HHWT", 
                                               other_grouping_vars = "homeownership",
                                               breakdowns = "total")


## of households that include someone with an ambulatory, self-care, visual, or auditory disability.
num_hh <- household_dis %>%
  filter(var_type == "estimate") %>%
  select(year, var_type, any_dis_any_4, any_dis_any)

save(individual_dis, 
     individual_dis_age_3,
     individual_dis_any_age_3, 
     individual_dis_age_4,
     individual_dis_age_9,
     individual_dis_income,
     individual_dis_homeownership,
     household_dis, 
     household_dis_income,
     household_dis_homeownership, 
     file = "interim_data/clean_disability_data.RData")
```

```{r create_income_table_HH, eval = FALSE}
# make tables for income and each disability category
income_and_physical <- survey_by_demog(disability_data, "dis_physical", "PERWT", 
                                       other_grouping_vars = "income_group")
income_and_independent <- survey_by_demog(disability_data, "dis_independent", "PERWT",
                                          other_grouping_vars = "income_group")
income_and_selfcare <- survey_by_demog(disability_data, "dis_selfcare","PERWT", 
                                       other_grouping_vars = "income_group")
income_and_vision <- survey_by_demog(disability_data, "dis_vision","PERWT", 
                                     other_grouping_vars = "income_group")
income_and_hearing <- survey_by_demog(disability_data, "dis_hearing", "PERWT", 
                                      other_grouping_vars = "income_group")
income_and_cognitive <- survey_by_demog(disability_data, "dis_cognitive", "PERWT", 
                                        other_grouping_vars = "income_group")
income_and_any <- survey_by_demog(disability_data, "dis_any", "PERWT",  
                                  other_grouping_vars = "income_group")
income_and_sensory <- survey_by_demog(disability_data, "dis_sensory", "PERWT",  
                                  other_grouping_vars = "income_group")

income_all <- income_and_hearing %>%
  bind_df(
    income_and_independent,
    income_and_physical,
    income_and_vision,
    income_and_any,
    income_and_cognitive,
    income_and_selfcare,
    income_and_sensory,
    by = c("FIPS", "year", "sex", "race", "var_type", "income_group")
  )

#cleanup
rm(income_and_any, income_and_hearing, income_and_independent, income_and_physical, income_and_selfcare, income_and_cognitive, income_and_vision, income_and_sensory)

income_all <- income_all %>%
  mutate(income_group = factor(income_group, levels = c("Under 20k", "20k–49.9k","50k–74.9k", "75k–99.9k", "100k–149.9k", "150k+")))

save(income_all, file= "interim_data/income_6_all_dis.RData")
```

```{r create_own_vs_rent_table, eval= FALSE}

homeownership_dis_physical <- survey_by_demog(disability_data, "dis_physical", "PERWT", 
                                       other_grouping_vars = "homeownership")
homeownership_dis_independent <- survey_by_demog(disability_data, "dis_independent", "PERWT",
                                          other_grouping_vars = "homeownership")
homeownership_dis_selfcare <- survey_by_demog(disability_data, "dis_selfcare","PERWT", 
                                       other_grouping_vars = "homeownership")
homeownership_dis_vision <- survey_by_demog(disability_data, "dis_vision","PERWT", 
                                     other_grouping_vars = "homeownership")
homeownership_dis_hearing <- survey_by_demog(disability_data, "dis_hearing", "PERWT", 
                                      other_grouping_vars = "homeownership")
homeownership_dis_cognitive <- survey_by_demog(disability_data, "dis_cognitive", "PERWT", 
                                      other_grouping_vars = "homeownership")
homeownership_dis_sensory  <- survey_by_demog(disability_data, "dis_sensory", "PERWT", 
                                              other_grouping_vars = "homeownership")
homeownership_dis_any <- survey_by_demog(disability_data, "dis_any", "PERWT",  
                                  other_grouping_vars = "homeownership")

homeownership_all <- homeownership_dis_physical %>%
  bind_df(
    homeownership_dis_independent,
    homeownership_dis_selfcare,
    homeownership_dis_vision,
    homeownership_dis_hearing,
    homeownership_dis_cognitive,
    homeownership_dis_any,
    homeownership_dis_sensory, 
    by = c("FIPS", "year", "sex", "race", "var_type", "homeownership")
  )

rm(homeownership_dis_independent,
    homeownership_dis_selfcare,
    homeownership_dis_vision,
    homeownership_dis_hearing,
    homeownership_dis_cognitive,
    homeownership_dis_any,
    homeownership_dis_sensory, 
   homeownership_dis_physical)

save(homeownership_all, file = "interim_data/homeownership_all_dis.RData")
```

```{r hh_homeownership, eval=FALSE}
#disability_data <- arrow::read_feather("interim_data/disability_data.feather")

hh_age_flag <- disability_data %>%
  group_by(year, SERIAL) %>%
  mutate(
    hh_over_65 = any(age >= 65, na.rm = TRUE),
    hh_over_80 = any(age >= 80, na.rm = TRUE)
  ) %>%
  ungroup()

disability_data_hh <- disability_data %>%
  filter(PERNUM == 1)

hh_homeownership_dis_physical    <- survey_by_demog(disability_data_hh, "any_dis_physical", "HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_independent <- survey_by_demog(disability_data_hh, "any_dis_independent", "HHWT",
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_selfcare    <- survey_by_demog(disability_data_hh, "any_dis_selfcare","HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_vision      <- survey_by_demog(disability_data_hh, "any_dis_vision","HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_hearing     <- survey_by_demog(disability_data_hh, "any_dis_hearing", "HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_cognitive   <- survey_by_demog(disability_data_hh, "any_dis_cognitive", "HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_any         <- survey_by_demog(disability_data_hh, "any_dis_any", "HHWT", 
                                                    other_grouping_vars = "homeownership")
hh_homeownership_dis_sensory     <- survey_by_demog(disability_data_hh, "any_dis_sensory", "HHWT", 
                                                    other_grouping_vars = "homeownership")

hh_homeownership_all <- hh_homeownership_dis_physical %>%
  bind_df(
    hh_homeownership_dis_independent,
    hh_homeownership_dis_selfcare,
    hh_homeownership_dis_vision,
    hh_homeownership_dis_hearing,
    hh_homeownership_dis_cognitive,
    hh_homeownership_dis_sensory, 
    hh_homeownership_dis_any,
    by = c("FIPS", "year", "sex", "race", "var_type", "homeownership")
  )

#clean
rm(hh_homeownership_dis_physical, hh_homeownership_dis_independent, 
   hh_homeownership_dis_selfcare, hh_homeownership_dis_vision, 
   hh_homeownership_dis_cognitive, hh_homeownership_dis_hearing, hh_homeownership_dis_any, hh_homeownership_dis_sensory)

save(hh_homeownership_all, file = "interim_data/hh_homeownership_all_dis.RData")
```

```{r hh_income, eval=FALSE}
# disability_data <- arrow::read_feather("interim_data/disability_data.feather")

disability_data_hh <- disability_data %>%
  filter(PERNUM == 1)

hh_income_dis_physical    <- survey_by_demog(disability_data_hh, "any_dis_physical", "HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_independent <- survey_by_demog(disability_data_hh, "any_dis_independent", "HHWT",
                                             other_grouping_vars = "income_group")
hh_income_dis_selfcare    <- survey_by_demog(disability_data_hh, "any_dis_selfcare","HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_vision      <- survey_by_demog(disability_data_hh, "any_dis_vision","HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_hearing     <- survey_by_demog(disability_data_hh, "any_dis_hearing", "HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_cognitive   <- survey_by_demog(disability_data_hh, "any_dis_cognitive", "HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_any         <- survey_by_demog(disability_data_hh, "any_dis_any", "HHWT", 
                                             other_grouping_vars = "income_group")
hh_income_dis_sensory     <- survey_by_demog(disability_data_hh, "any_dis_sensory", "HHWT", 
                                             other_grouping_vars = "income_group")

hh_income_all <- hh_income_dis_physical %>%
  bind_df(
    hh_income_dis_independent,
    hh_income_dis_selfcare,
    hh_income_dis_vision,
    hh_income_dis_hearing,
    hh_income_dis_cognitive,
    hh_income_dis_any,
    hh_income_dis_sensory, 
    by = c("FIPS", "year", "sex", "race", "var_type", "income_group")
  )
  
#clean
rm(hh_income_dis_any, hh_income_dis_cognitive, hh_income_dis_hearing, ... = hh_income_dis_independent, hh_income_dis_physical, hh_income_dis_selfcare, hh_income_dis_sensory, hh_income_dis_vision)

hh_income_all <- hh_income_all %>%
  mutate(income_group = factor(income_group, levels = c("Under 20k", "20k–49.9k","50k–74.9k", "75k–99.9k", "100k–149.9k", "150k+")))

save(hh_income_all, file = "interim_data/hh_income_all_dis.RData")
```

```{r create_age_table_1, eval=FALSE}

#make big table 
group_var <- "age_group_4"

age_and_physical     <- survey_by_demog(disability_data, "dis_physical", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_independent  <- survey_by_demog(disability_data, "dis_independent", "PERWT",
                                        other_grouping_vars = group_var)
age_and_selfcare     <- survey_by_demog(disability_data, "dis_selfcare", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_vision       <- survey_by_demog(disability_data, "dis_vision", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_hearing      <- survey_by_demog(disability_data, "dis_hearing", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_cognitive    <- survey_by_demog(disability_data, "dis_cognitive", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_sensory      <- survey_by_demog(disability_data, "dis_sensory", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_any          <- survey_by_demog(disability_data, "dis_any", "PERWT",  
                                        other_grouping_vars = group_var)

age_all <- age_and_hearing %>%
  bind_df(
    age_and_independent,
    age_and_physical,
    age_and_vision,
    age_and_any,
    age_and_cognitive,
    age_and_selfcare,
    age_and_sensory,
    by = c("FIPS", "year", "sex", "race", "var_type", group_var)
  )

#factorize
age_all <- age_all %>%
  mutate(age_group_4 = factor(age_group_4, levels = c("Under 50", "50-64", "65-80", "80 or Older"), ordered = TRUE))

rm(age_and_any, age_and_hearing, age_and_independent, age_and_physical, age_and_selfcare, age_and_vision, age_and_sensory)

save(age_all, file = "interim_data/age_4_all_dis.RData")
```

```{r create_age_table_2, eval=FALSE}

#make big table 
group_var <- "age_group_5_yrs"

age_and_physical     <- survey_by_demog(disability_data, "dis_physical", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_independent  <- survey_by_demog(disability_data, "dis_independent", "PERWT",
                                        other_grouping_vars = group_var)
age_and_selfcare     <- survey_by_demog(disability_data, "dis_selfcare", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_vision       <- survey_by_demog(disability_data, "dis_vision", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_hearing      <- survey_by_demog(disability_data, "dis_hearing", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_cognitive    <- survey_by_demog(disability_data, "dis_cognitive", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_any          <- survey_by_demog(disability_data, "dis_any", "PERWT",
                                        other_grouping_vars = group_var)
age_and_sensory          <- survey_by_demog(disability_data, "dis_sensory", "PERWT",
                                        other_grouping_vars = group_var)

age_all_5 <- age_and_hearing %>%
  bind_df(
    age_and_independent,
    age_and_physical,
    age_and_vision,
    age_and_cognitive,
    age_and_any,
    age_and_selfcare,
    age_and_sensory, 
    by = c("FIPS", "year", "sex", "race", "var_type", group_var)
  )

age_all_5 <- age_all_5 %>%
  mutate(age_group_5_yrs = factor(age_group_5_yrs, levels = c("50-54", "55-59","60-64", "65-69", "70-74", "75-79", "80-84", "85-90", "90 plus"), ordered = TRUE))

rm(age_and_any, age_and_hearing, age_and_independent, age_and_physical, age_and_cognitive, age_and_selfcare, age_and_vision, age_and_sensory)

save(age_all_5, file = "interim_data/age_5_all_dis.RData")

```

```{r create_age_table_3, eval=FALSE}

#make big table 
group_var <- "age_group_2"

age_and_physical     <- survey_by_demog(disability_data, "dis_physical", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_independent  <- survey_by_demog(disability_data, "dis_independent", "PERWT",
                                        other_grouping_vars = group_var)
age_and_selfcare     <- survey_by_demog(disability_data, "dis_selfcare", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_vision       <- survey_by_demog(disability_data, "dis_vision", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_hearing      <- survey_by_demog(disability_data, "dis_hearing", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_cognitive    <- survey_by_demog(disability_data, "dis_cognitive", "PERWT", 
                                        other_grouping_vars = group_var)
age_and_any          <- survey_by_demog(disability_data, "dis_any", "PERWT",
                                        other_grouping_vars = group_var)
age_and_sensory          <- survey_by_demog(disability_data, "dis_sensory", "PERWT",
                                        other_grouping_vars = group_var)

age_all_2 <- age_and_hearing %>%
  bind_df(
    age_and_independent,
    age_and_physical,
    age_and_vision,
    age_and_cognitive,
    age_and_any,
    age_and_selfcare,
    age_and_sensory, 
    by = c("FIPS", "year", "sex", "race", "var_type", group_var)
  )

age_all_2 <- age_all_2 %>%
  mutate(age_group_2 = factor(age_group_2, levels = c("Under 65", "Over 65"), ordered = TRUE))

rm(age_and_any, age_and_hearing, age_and_independent, age_and_physical, age_and_cognitive, age_and_selfcare, age_and_vision, age_and_sensory)

save(age_all_2, file = "interim_data/age_2_all_dis.RData")

```

```{r skip, eval=FALSE}
## @MB: Processing by hand

# @MB You asked to see the code that does the work in the above.
# The more specific function is glptools:::svy_repwts. It's long because it creates multiple different outputs (i.e. not only does it make the number we're looking for, but also the MOE, confidence interval, etc.)
# The actual code that summarizes the data is:
#
# results_estimate %<>%
#   group_by(across(all_of(c(grouping_vars, var)))) %>%
#   summarise(estimate = sum(.data[[weight_var]]), .groups = "drop")

# To write that with the specific variables used here: 
# This filters to rows where none of the columns specified in that character vector are NA.
# It can be complicated to figure out when to use quoted vs. unquoted variable names. While filter normally works with unquoted variables, I need to use quoted strings for Dplyr to use if_all to find them in the data frame, then apply the formula ~!is.na(.) to them all.

example_df <- disability_data %>%
  filter(if_all(c("FIPS", "year", "sex", "race", "dis_physical"), ~!is.na(.))) 

# Count the number of cases for each value of dis_physical within each demographic category.
example_df %<>%
  group_by(FIPS, year, sex, race, dis_physical) %>%
  summarise(estimate = sum(PERWT), .groups = "drop") #You can either use ungroup() the function, or specify .groups = "drop" to ungroup data.

# Summarize the data and create a percentage
example_df %<>%
  
  # Note that I'm not grouping by dis_physical here. This creates "mini-data frames" where each one has exactly three rows: one for each value of dis_physical. (T, F, & NA)
  group_by(FIPS, year, sex, race) %>% 
  
  mutate(
    population = sum(estimate), # Add together cases where dis_physical is TRUE and dis_physical is FALSE. 
    percent  = estimate / population * 100) %>%
  ungroup()

example_df %<>%
  filter(dis_physical) %>% # I just want the rows where this is true
  select(-dis_physical)

total_over_65_80 <- disability_data %>%
  mutate(
    over_65 = age >= 65,
    over_80 = age >= 80
  ) %>%
  group_by(year) %>%
  summarise(
    pop_over_65 = sum(PERWT * over_65, na.rm = TRUE),
    pop_over_80 = sum(PERWT * over_80, na.rm = TRUE)
  )
```

```{r skip2, eval = FALSE}
# ## Revisiting some things
# 
# survey_by_demog is mainly a wrapper function that calls svy_repwts or svy_bootstrap multiple times and combines the results.
# 
# The notation in R to access a function without loading in a package (or to specify the package to address conflicts in the namespace) is a double colon. Some packages, including glptools, include functions that are not loaded into the namespace at all. Those are available via triple colon.

# The confidence intervals tend to fall apart at small values.
# Bootstrapping is a method used to generate confidence intervals by re-sampling the observations themselves. It does discard some information the Census bakes in via their replicate weights, but notably, it provides separate numbers for the lower bound and upper bound, rather than one MOE and CI.

# Bootstrapping a year of data in Louisville, for example, looks like taking the observations for Louisville, taking 1,000 samples from those observations, then looking at the spread of those samples

# I am largely unable to follow my own code to create bootstrap weights at this point. It uses the map_ functions, which are a part of the purrr package of the tidyverse. The map_ functions can typically replace for loops in R, offer performance benefits, and allow you to write shorter code. However, they come with a level of incredible complexity that I can't follow any more. You're doing a lot "behind the curtain" kind of like you can do with group_by() %>% summarize(), but it extends beyond doing simple math on each individual group and extends it to more complex operations.

# individual_disability <- survey_by_demog(filter(disability_data, year == 2024), 
#                                          "dis_physical", 
#                                          "PERWT",
#                                          method = "bootstrap",
#                                          breakdowns = "total")

```

```{r poverty_rates,eval=FALSE}
# Poverty rate, any disability present in household vs not.
#load(file = "data/acs_micro_subset_2024.RData")

overall_poverty <- disability_data %>%
  summarise(overall_poverty = weighted.mean(poverty, HHWT, na.rm = TRUE))

poverty_hh_dis <- disability_data %>%
  # dis_any_4 = dis_physical + dis_selfcare + dis_vision + dis_hearing
  filter(!is.na(any_dis_any_4)) %>%
  group_by(any_dis_any_4) %>%
  summarise(poverty = weighted.mean(poverty, HHWT, na.rm = TRUE))

# Poverty rate, ambulatory disability present in household vs not. 
poverty_hh_amb <- disability_data %>%
  filter(!is.na(any_dis_physical)) %>%
  group_by(any_dis_physical) %>%
  summarise(poverty = weighted.mean(poverty, HHWT, na.rm = TRUE))
```

# American Community Survey (ACS)

The following section summarizes findings from ACS data on disability prevalence in Louisville, KY, highlighting how factors like race, sex, income, and age relate to disability rates.

<details>

<summary><i> See variable definitions </i></summary>

```{r, echo=FALSE, eval=TRUE}
library(knitr)
library(kableExtra)
defs <- data.frame(
  Variable = c("Ambulatory difficulty",
    "Independent living difficulty",
    "Self-care difficulty",
    "Vision difficulty",
    "Hearing difficulty", 
    "Cognitive difficulty"),
  Description = c("DIFFPHYS- Indicates whether the respondent has a condition that substantially limits one or more basic physical activities, such as walking, climbing stairs, reaching, lifting, or carrying.",
                  "DIFFMOB- Indicates whether the respondent has any physical, mental, or emotional condition lasting six months or more that makes it difficult or impossible to perform basic activities outside the home alone. Does not include temporary health problems.",
                  "DIFFCARE- Indicates whether respondents have any physical or mental health condition that has lasted at least six months and makes it difficult to take care of their own personal needs (bathing, dressing, getting around inside the home). Excludes temporary conditions.",
                  "DIFFEYE- Indicates whether the respondent is blind or has serious difficulty seeing even with corrective lenses.",
                  "DIFFHEAR- Indicates whether the respondent is deaf or has serious difficulty hearing.", 
                  "DIFFREM- Indicates whether the respondent has cognitive difficulties (such as learning, remembering, concentrating, or making decisions) because of a physical, mental, or emotional condition."))

#convert data frame to markdown table 
kable(defs, "html", escape = FALSE, col.names = c("Variable", "Description")) %>%
  kable_styling(
    full_width = FALSE,
    position = "center",
    #alternate row colors, reduce padding, scroll if on mobile
    bootstrap_options = c("striped", "condensed", "responsive")
  )
```

</details>

## Trends Across Demographics (2024)

### Overall {.tabset}

#### Individual

```{r prevalence_chart_indv}

load("interim_data/clean_disability_data.RData")
showtext_auto()

#mutate dfs to plot
df_all <- individual_dis %>%
  filter(sex == "total", race == "total", var_type %in% c("estimate", "population")) %>%
  select(-FIPS, -sex, -race) %>%
  pivot_longer(dis_physical:dis_sensory) %>%
  pivot_wider(names_from = var_type, values_from = value)

df_all %<>%
  group_by_at(df_all %cols_in% c("age_group_4", "name", "year")) %>%
  summarise(
    estimate = sum(estimate, na.rm = TRUE),
    population = sum(population, na.rm = TRUE)) %>%
  ungroup()

df_all <- df_all %>%
  left_join(std_age, by = "age_group_4") %>%
  group_by(name, year) %>%
  summarise(rate = sum(estimate / population * weight)*100, 
            total_population = sum(population, na.rm = TRUE), 
            estimate = sum(estimate, na.rm = TRUE)) %>%
  ungroup()

overall_excel <- df_all %>%
  filter(year == 2024) %>%
  rename(Category = name, Percent = rate, Number = estimate) %>%
  mutate(Category = recode(Category, 
                                  dis_physical = "Physical",
                                  dis_sensory = "Sensory",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  select(Category, Percent, Number)

save(df_all, file = "interim_data/age_adjusted_all.RData")

# chart 1 -> prevalence of disabilities by type
# only most recent year (can change)
dis_prev_by_type <- df_all %>% 
  filter(year == 2024) %>%
  mutate(Disability_Type = recode(name, 
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  rename(Percent = rate) %>%
  select(Disability_Type, Percent)

save(dis_prev_by_type, file= "interim_data/dis_prev_by_type.Rdata")

dis_prev_by_type$Percent <- round(dis_prev_by_type$Percent, 2)

plot1 <- ggplot(dis_prev_by_type, aes(x = Disability_Type, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")), 
            vjust = -0.5, size = 6) +
  labs(
    title = "Prevalence of Disabilities by Type (2024)",
    x = NULL,
    y = "Percent of Population"
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
  )
  print(plot1)

  ggsave("interim_data/plots/dis_by_type.png", plot= plot1)
  
```

#### Household

```{r prevalence_chart_HH}
load(file = "interim_data/clean_disability_data.RData")
showtext_auto()


HH_prev <- household_dis %>%
  filter(year == 2024, var_type == "percent") %>%
  pivot_longer(cols = c(any_dis_physical, any_dis_independent, any_dis_selfcare, any_dis_vision, any_dis_cognitive, any_dis_hearing, any_dis_any,  any_dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  mutate(Disability_Type = recode(Disability_Type, any_dis_physical = "Physical",
                                  any_dis_independent = "Ind. Living",
                                  any_dis_selfcare = "Self-Care",
                                  any_dis_vision = "Vision",
                                  any_dis_hearing = "Hearing",
                                  any_dis_sensory = "Sensory",
                                  any_dis_cognitive = "Cognitive",
                                  any_dis_any = "Any")) %>%
  select(Disability_Type, Percent) 


save(HH_prev, file= "interim_data/HH_prev.Rdata")
	
HH_prev$Percent <- round(HH_prev$Percent, 2)

HH_prev_plot <- ggplot(HH_prev, aes(x = Disability_Type, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(Percent, "%")), vjust = -0.5, size = 6) +
  labs(
    title = "Households with a Person with a Disability by Type (2024)", 
    subtitle = "Household Level", 
    x = NULL,
    y = "Percent of Households"
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1,family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )
  print(HH_prev_plot)

  ggsave("interim_data/plots/HH_prev_plot.png", plot= HH_prev_plot)
```

### Sex

```{r sex_chart}

showtext_auto()
colors <- viridis::viridis(2)
#c("#f60", "#FFCCC2")

sex_excel <- individual_dis %>%
filter(year ==2024, var_type %in% c("percent", "estimate", "population"), sex != "total", race == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  left_join(std_age, by = "age_group_4") %>%
  group_by(Disability_Type, sex) %>%
  summarise(rate = sum(estimate / population * weight)*100, 
            total_population = sum(population, na.rm = TRUE), 
            estimate = sum(estimate, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(Category = recode(Disability_Type,
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any"), 
         sex = recode(sex, female = "Female", male = "Male")) %>%
  select(sex, Category, everything() , -total_population, -Disability_Type) %>%
  rename(Sex = sex, Number = estimate, Percent = rate) 

full_sex_excel <- sex_excel %>%
  pivot_wider(
    names_from = Sex,
    values_from = c(Percent, Number),
    names_glue = "{tolower(Sex)}_{.value}"
  )
  
#create chart
plot2 <- ggplot(sex_excel, aes(x = Category, y = Percent, fill = Sex)) +  # fill says split x category by sex
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Sex (2024)",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )
  print(plot2)
  ggsave("interim_data/plots/dis_by_sex.png", plot= plot2)
  
```

### Race

```{r race_chart}
load(file = "interim_data/age_adjusted.rData")


race_excel <- individual_dis %>%
filter(year ==2024, var_type %in% c("percent", "estimate", "population"), race != "total", sex == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  left_join(std_age, by = "age_group_4") %>%
  group_by(Disability_Type, race) %>%
  summarise(rate = sum(estimate / population * weight)*100, 
            total_population = sum(population, na.rm = TRUE), 
            estimate = sum(estimate, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(race != "other") %>%
  mutate(Category = recode(Disability_Type,
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any"),
         Race = recode(race, 
                      black = "Black", 
                      hispanic = "Hispanic", 
                      white = "White")) %>%
  select(Race, Category, everything() , -total_population, -Disability_Type, -race) %>%
  rename(Number = estimate, Percent = rate)

full_race_excel <- race_excel %>%
  pivot_wider(
    names_from = Race,
    values_from = c(Percent, Number),
    names_glue = "{tolower(Race)}_{.value}"
  )

showtext_auto()
colors <- viridis::viridis(4)
  #colorRampPalette(c("#FFCCC2", "#cc3f14"))(4)

  #clean table
dis_by_race <- df %>%
  filter(year == 2024, race != "total", race != "other") %>%
  mutate(Disability_Type = recode(name,
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_cognitive = "Cognitive",
                                  dis_sensory = "Sensory",
                                  dis_any = "Any")) %>%
  select(-name, year)%>%
  mutate(race = recode(race, 
                      black = "Black", 
                      hispanic = "Hispanic", 
                      white = "White", 
                      other = "Not Listed"), 
         race = factor(race, levels = c("Black", "White", "Hispanic"))) %>% 
  rename(Race = race, Percent = rate)
  
  #make chart
plot3 <- ggplot(dis_by_race, aes(x = Disability_Type, y = Percent, fill = Race)) +  # fill says split x category by Race
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Race (2024)",
    x = NULL, y = "Percent of Population"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/dis_by_race.png", plot= plot3)
  print(plot3)
```

### Own vs. Rent {.tabset}

#### Individual

```{r homeownership_individual_graph}
load("interim_data/homeownership_all_dis.RData")

showtext_auto()
colors <-  viridis::viridis(2)
#c("#f60", "#FFCCC2")

#clean it
dis_by_own <- homeownership_all %>%
  filter(year == 2024, var_type == "percent", sex == "total", race == "total", !is.na(homeownership)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(homeownership, Disability_Type,Percent) %>%
  mutate(
    Disability_Type = recode(Disability_Type,
                             dis_physical = "Physical",
                             dis_independent = "Ind. Living",
                             dis_selfcare = "Self-Care",
                             dis_vision = "Vision",
                             dis_hearing = "Hearing",
                             dis_sensory = "Sensory",
                             dis_cognitive = "Cognitive",
                             dis_any = "Any"),
    Homeownership = if_else(homeownership, "Owns", "Rents")
  ) %>%
  select(-homeownership)

#chart it!
plot6 <- ggplot(dis_by_own, aes(x = Disability_Type, y = Percent, fill = Homeownership)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Homeownership (2024)",
    x = NULL, y = "Percent of Population"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/dis_by_homeownership.png", plot= plot6)
  print(plot6)

```

#### Household

```{r hh_homeownership_graph}

showtext_auto()
load("interim_data/hh_homeownership_all_dis.RData")
colors <- viridis::viridis(2)
#c("#f60", "#FFCCC2")

#clean it
hh_dis_by_own <- hh_homeownership_all %>%
  filter(year == 2023, var_type == "percent", sex == "total", race == "total", !is.na(homeownership)) %>%
  pivot_longer(cols = c(any_dis_physical, any_dis_independent, any_dis_selfcare, any_dis_vision, any_dis_hearing, any_dis_cognitive, any_dis_any, any_dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(homeownership, Disability_Type,Percent) %>%
  mutate(
    Disability_Type = recode(Disability_Type,
                             any_dis_physical = "Physical",
                             any_dis_independent = "Ind. Living",
                             any_dis_selfcare = "Self-Care",
                             any_dis_vision = "Vision",
                             any_dis_hearing = "Hearing",
                             any_dis_cognitive = "Cognitive",
                             any_dis_sensory = "Sensory", 
                             any_dis_any = "Any"),
    Homeownership = if_else(homeownership, "Owns", "Rents")
  ) %>%
  select(-homeownership)

#chart it!
hh_own_rent <- ggplot(hh_dis_by_own, aes(x = Disability_Type, y = Percent, fill = Homeownership)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Homeownership (2024)",
    subtitle = "Household Level", 
    x = NULL, y = "Percent of Households"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/hh_dis_by_homeownership.png", plot= hh_own_rent)
  print(hh_own_rent)

```

### Income {.tabset}

#### Individual

```{r income_chart}
load("interim_data/income_6_all_dis.RData")
showtext_auto()
colors <- viridis::viridis(6)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(6)

income_excel <- income_all %>%
  filter(year == 2024, var_type %in% c("percent","estimate", "population"), sex == "total", race == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Category", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  mutate(Category = recode(Category, dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_cognitive = "Cognitive",
                                  dis_sensory = "Sensory", 
                                  dis_any = "Any")) %>%
  rename(Income = income_group, Number = estimate, Percent = percent) %>%
  select(-FIPS, -race, -sex, -year) %>%
  arrange(Category, Income)

full_income_excel <- income_excel %>%
  pivot_wider(
    names_from = Income,
    values_from = c(Percent, Number),
    names_glue = "{tolower(Income)}_{.value}"
  )

#clean it
dis_by_income <- income_all %>%
  filter(year == 2024, var_type == "percent", sex == "total", race == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(income_group, Disability_Type,Percent) %>%
  mutate(Disability_Type = recode(Disability_Type, 
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory", 
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
rename("Income Group" = income_group)

#chart it!
plot4 <- ggplot(dis_by_income, aes(x = Disability_Type, y = Percent, fill = `Income Group`)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Income (2024)",
    x = NULL, y = "Percent of Population Per Income Group"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/dis_by_income.png", plot= plot4)
  print(plot4)
  
```

#### Household

```{r hh_income_graph}

showtext_auto()
load("interim_data/hh_income_all_dis.RData")
colors <- viridis::viridis(6)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(6)

hh_income_excel <- hh_income_all %>%
  filter(year == 2024, var_type %in% c("percent","estimate"), sex == "total", race == "total") %>%
  pivot_longer(cols = c(any_dis_physical, any_dis_independent, any_dis_selfcare, any_dis_vision, any_dis_hearing, any_dis_cognitive, any_dis_any, any_dis_sensory),
               names_to = "Category", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  mutate(Category = recode(Category, dis_physical = "Physical",
                                  any_dis_independent = "Ind. Living",
                                  any_dis_selfcare = "Self-Care",
                                  any_dis_vision = "Vision",
                                  any_dis_hearing = "Hearing",
                                  any_dis_sensory = "Sensory",
                                  any_dis_cognitive = "Cognitive",
                                  any_dis_any = "Any")) %>%
  rename(Income = income_group, Number = estimate, Percent = percent) %>%
  select(-FIPS, -race, -sex, -year) %>%
  arrange(Category, Income)

full_hh_income_excel <- hh_income_excel %>%
  pivot_wider(
    names_from = Income,
    values_from = c(Percent, Number),
    names_glue = "{tolower(Income)}_{.value}"
  )

#clean it
hh_dis_by_own <- hh_income_all %>%
  filter(year == 2024, var_type == "percent", sex == "total", race == "total", !is.na(income_group)) %>%
  pivot_longer(cols = c(any_dis_physical, any_dis_independent, any_dis_selfcare, any_dis_vision, any_dis_hearing, any_dis_cognitive, any_dis_any, any_dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(income_group, Disability_Type, Percent) %>%
  mutate(
    Disability_Type = recode(Disability_Type,
                             any_dis_physical = "Physical",
                             any_dis_independent = "Ind. Living",
                             any_dis_selfcare = "Self-Care",
                             any_dis_vision = "Vision",
                             any_dis_hearing = "Hearing",
                             any_dis_sensory = "Sensory",
                             any_dis_cognitive = "Cognitive",
                             any_dis_any = "Any")) %>%
  rename("Household income" = income_group)

#chart it!
hh_income <- ggplot(hh_dis_by_own, aes(x = Disability_Type, y = Percent, fill = `Household income`)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Income (2024)",
    subtitle = "Household Level", 
    x = NULL, y = "Percent of Households"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/hh_dis_by_income.png", plot= hh_income)
  print(hh_income)

```

### Age {.tabset}

#### All Ages

```{r age_chart}
load(file = "interim_data/age_2_all_dis.RData")
load(file = "interim_data/age_4_all_dis.RData")

showtext_auto()
colors <- viridis::viridis(2)
# colorRampPalette(c("#FFCCC2", "#cc3f14"))(4)
# c("#f60", "#FFCCC2")


age_excel <- age_all_2 %>%
  filter(year == 2024, sex == "total", race == "total", var_type %in% c("percent", "estimate"), !is.na(age_group_2)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Category", 
               values_to = "Value") %>%
  pivot_wider(names_from = var_type, values_from = Value) %>%
  mutate(Category = recode(Category, dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  rename(Age = age_group_2, Number = estimate, Percent = percent) %>%
  select(-FIPS, -sex, -race, -year) %>%
  arrange(Category) %>%
  pivot_wider(names_from = Age, values_from = c("Number", "Percent"))


# full_age_excel <- age_excel %>%
#   pivot_wider(
#     names_from = Age,
#     values_from = c(Percent, Number),
#     names_glue = "{tolower(Age)}_{.value}"
#   )

# over 65 vs under
age_new <- age_all_2 %>%
   filter(year == 2024, sex == "total", race == "total", var_type == "percent", !is.na(age_group_2))%>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent")%>%
  mutate(Disability_Type = recode(Disability_Type, dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory", 
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  rename(Age = age_group_2) %>%
  mutate(Age = factor(Age, levels = c("Under 65", "Over 65"))) %>%
  select(Age, Disability_Type, Percent) 

#chart it!
plot5 <- ggplot(age_new, aes(x = Disability_Type, y = Percent, fill = Age)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Age (2024)",
    x = NULL, y = "Percent of Population Per Age Group"
  ) +
  scale_fill_manual(values = viridis::viridis(2)) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/dis_by_age.png", plot= plot5)
  print(plot5)
```

#### Older Subset

```{r age_5yr_chart}
load(file = "interim_data/age_5_all_dis.RData")

showtext_auto()
colors <- viridis::viridis(9)
  #colorRampPalette(c("#FFCCC2", "#cc3f14"))(9)

#clean table
dis_by_age <- age_all_5 %>%
  filter(year == 2024, sex == "total", race == "total", var_type == "percent",!is.na(age_group_5_yrs)) %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent") %>% 
  mutate(Disability_Type = recode(Disability_Type, dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_sensory = "Sensory",
                                  dis_cognitive = "Cognitive",
                                  dis_any = "Any")) %>%
  select(age_group_5_yrs, Disability_Type, Percent) %>% 
  rename(Age = age_group_5_yrs)

plot5yr <- ggplot(dis_by_age, aes(x = Disability_Type, y = Percent, fill = Age)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
    title = "Disabilities Breakdown by Age- 50+ (2024)",
    x = NULL, y = "Percent of Population"
  ) +
  scale_fill_manual(values = colors) +  
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat")
  )

  ggsave("interim_data/plots/dis_by_age_5yr.png", plot= plot5yr)
  print(plot5yr)
```

### Map

```{r}

library(sf)

dis_vars <- build_census_var_df("acs5", "B18105")

dis_vars %<>% filter(year == 2024)

dis_df <- get_census(dis_vars, "tract")

dis_df %<>%
  mutate(
    dis_phys = case_when(
      str_detect(label, "With an ambulatory difficulty") ~ T,
      str_detect(label, "No ambulatory difficulty") ~ F,
      T ~ NA))

dis_df2 <- dis_df %>%
  process_census(cat_var = "dis_phys", output_name = "ambulatory_dis")

process_map(dis_df2, ambulatory_dis, return_name = "dis_phys", maps = "nh") %>% list2env(.GlobalEnv)

dis_phys_nh %<>%
  filter(sex == "total", var_type == "percent")

map_nh <- st_as_sf(glptools::map_nh)

dis_map <- left_join(map_nh, dis_phys_nh, by = "neighborhood")

dis_map %<>%
  select(neighborhood, ambulatory_dis)


```

```{r dis_map_plot}
plotmap <- ggplot(dis_map) +
  geom_sf(aes(fill = ambulatory_dis), color = "white", linewidth = 0.5) +
  scale_fill_viridis_c(
    name = "Physical\nDisability Rate (%)",
    option = "viridis",
    limits = c(0, 16),
    breaks = c(0, 4, 8, 12, 16),
    labels = c("0%", "4%", "8%", "12%", "16%")
  ) +
  labs(
    title = "Physical disability by Neighborhood",
  ) +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 30, face = "bold", family = "Montserrat"),
    legend.position = "right",
    legend.text = element_text(hjust = 1, size = 16, family = "Montserrat"),
    legend.title = element_text(size = 20, family = "Montserrat")
  )

  ggsave("interim_data/plots/plotmap.png", plot= plotmap)
  print(plotmap)
```


## Trends Over Time (2008-2024)

*Note missing data for the year 2020*

### Overall

```{r overtime_overall}
save(df_all, file = "interim_data/age_adjusted_all.RData")

showtext_auto()
colors <- viridis::viridis(8)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(6)


all_color <- "#52b899"

dis_prev_by_type_over_time <- df_all %>%
  mutate(Disability_Type = recode(name, 
                                  dis_physical = "Physical",
                                  dis_independent = "Ind. Living",
                                  dis_selfcare = "Self-Care",
                                  dis_vision = "Vision",
                                  dis_hearing = "Hearing",
                                  dis_cognitive = "Cognitive", 
                                  dis_sensory = "Sensory",
                                  dis_any = "Any")) %>%
  rename(Percent = rate) %>%
  select(-name) %>%
  select(year, Disability_Type, Percent)

library(ggrepel)
#for adding labels

grid_lines_y <- tibble(
  y = seq(0, max(dis_prev_by_type_over_time$Percent), by = 2.5),
  x1 = min(dis_prev_by_type_over_time$year),
  x2 = 2024
)

last_points <- dis_prev_by_type_over_time %>%
  group_by(Disability_Type) %>%
  filter(year == max(year)) %>%
  ungroup()%>% 
  mutate(
    Label = sprintf("%s\n%.1f%%", Disability_Type, Percent))

plot_overall_time <- ggplot(dis_prev_by_type_over_time, aes(x = year, y = Percent, color = Disability_Type, group = Disability_Type)) +
  geom_segment(
    data = tibble(x = seq(min(dis_prev_by_type_over_time$year), 2024, by = 1),
                  y1 = 0,
                  y2 = max(dis_prev_by_type_over_time$Percent)),
    aes(x = x, xend = x, y = y1, yend = y2),
    inherit.aes = FALSE,
    color = "grey91",
    size = 0.4
) + geom_segment(
  data = tibble(x = seq(min(dis_prev_by_type_over_time$year), 2024, by = 2),
                  y1 = 0,
                  y2 = max(dis_prev_by_type_over_time$Percent)),
    aes(x = x, xend = x, y = y1, yend = y2),
    inherit.aes = FALSE,
    color = "grey91",
    size = 0.8
) +
  geom_segment(
  data = filter(grid_lines_y, y %% 5 == 0),
  aes(x = x1, xend = x2, y = y, yend = y),
  inherit.aes = FALSE,
  color = "grey91",
  size = 0.8
) +
geom_segment(
  data = filter(grid_lines_y, y %% 5 != 0),
  aes(x = x1, xend = x2, y = y, yend = y),
  inherit.aes = FALSE,
  color = "grey91",
  size = 0.4
) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)              
  labs(
    title = "Disability Trends Over Time",
    x = NULL,
    y = "Percent of Population"
  ) +
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(dis_prev_by_type_over_time$year),max(dis_prev_by_type_over_time$year), by = 2),  #every other year
    limits = c(min(dis_prev_by_type_over_time$year), 2025)
  ) +
  scale_color_manual(values = colors) +
  geom_text_repel(
    data = last_points,
    aes(label = Label),
    nudge_x = 1.0, # push labels into white space
    hjust = 0,
    direction = "y",
    family = "Montserrat",
    fontface = "bold",
    lineheight = 0.5,
    size = 8,
    segment.size = 0.6,
    segment.alpha = 0.5,
    segment.linetype = "dotted",
    box.padding = 0.4
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "none", 
    panel.grid.major.y = element_blank(),  # remove horizontal grid
    panel.grid.minor.y = element_blank(), 
    panel.grid.major.x = element_blank(), 
    panel.grid.minor.x = element_blank()
  )

#ggsave("interim_data/plots/dis_prev_by_type_over_time.png", plot = plot_overall_time)
print(plot_overall_time)


```

### Sex

```{r sex_chart_time}
load(file = "interim_data/clean_disability_data.RData")

#age adjusted data
df_sex <- individual_dis %>%
  filter(race == "total", var_type %in% c("estimate", "population")) %>%
  select(-FIPS, -race) %>%
  pivot_longer(dis_physical:dis_any) %>%
  pivot_wider(names_from = var_type, values_from = value)

df_sex %<>% 
  group_by_at(df_sex %cols_in% c("age_group_4", "sex", "name", "year")) %>%
  summarise(
    estimate = sum(estimate, na.rm = TRUE),
    population = sum(population, na.rm = TRUE))%>%
  ungroup()

df_sex <- df_sex %>%
  left_join(std_age, by = "age_group_4") %>%
  group_by(sex, name, year) %>%
  summarise(rate = sum(estimate / population * weight)*100) %>%
  ungroup()

showtext_auto()
colors <- viridis::viridis(2)
#c("#f60", "#FFCCC2")

sex_over_time <- df_sex %>%
  filter(sex != "total", name == "dis_physical") %>%
  mutate(year = as.numeric(year)) %>%
  select(-name) %>%
  rename(Sex = sex, dis_physical = rate)

plot_sex_time <- ggplot(sex_over_time, aes(x = year, y = dis_physical, color = Sex, group = Sex)) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)                      
  labs(
    title = "Physical Disability Over Time by Sex",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_color_manual(values = colors) +
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(sex_over_time$year), max(sex_over_time$year), by = 2)  #every other year
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )

ggsave("interim_data/plots/dis_physical_by_sex_time.png", plot = plot_sex_time)
print(plot_sex_time)
```

### Race

```{r race_chart_time}
load(file = "interim_data/clean_disability_data.RData")
showtext_auto()
colors <- viridis::viridis(4)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(4)

# TODO - remove not listed from graph
race_over_time <- df %>%
  filter(race != "total", name == "dis_physical", race != "other") %>%
  select(-name, year)%>%
  mutate(race = recode(race, 
                      black = "Black", 
                      hispanic = "Hispanic", 
                      white = "White"), 
         race = factor(race, levels = c("Black", "White", "Hispanic"))) %>% 
  rename(Race = race, dis_physical = rate)
  
plot_race_time <- ggplot(race_over_time, aes(x = year, y = dis_physical, color = Race, group = Race)) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)                      
  labs(
    title = "Physical Disability Over Time by Race",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_color_manual(values = colors) +
  theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(race_over_time$year), max(race_over_time$year), by = 2) 
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )

ggsave("interim_data/plots/dis_physical_by_race_time.png", plot = plot_race_time)
print(plot_race_time)
```

### Own Vs Rent

```{r own_vs_rent_time}
load(file = "interim_data/clean_disability_data.RData")
showtext_auto()
colors <- viridis::viridis(2)
#c("#f60", "#FFCCC2")

#phys_disability and race over time
homeownership_over_time<- individual_dis_homeownership %>%
  filter(race == "total", sex == "total", !is.na(homeownership), var_type == "percent") %>%
  select(year, homeownership, dis_physical) %>%
  mutate(Homeownership = if_else(homeownership, "Owns", "Rents")
  ) %>%
  select(-homeownership)

plot_owns_time <- ggplot(homeownership_over_time, aes(x = year, y = dis_physical, color = Homeownership, group = Homeownership)) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)                      
  labs(
    title = "Physical Disability Over Time by Homeownership",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_color_manual(values = colors) +
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(homeownership_over_time$year), max(homeownership_over_time$year), by = 2) 
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )

ggsave("interim_data/plots/dis_physical_by_owns_time.png", plot = plot_owns_time)
print(plot_owns_time)
```

### Income

```{r income_graph_time}
load(file = "interim_data/clean_disability_data.RData")
showtext_auto()
income_levels <- c("Under 20k", "20k–49.9k","50k–74.9k", "75k–99.9k", "100k–149.9k", "150k+")

individual_dis_income <- individual_dis_income %>%
  mutate(income_group = factor(income_group, levels = income_levels))

colors <- viridis::viridis(6)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(6)

income_over_time<- individual_dis_income %>%
  filter(race == "total", sex == "total", var_type == "percent") %>%
  select(year, income_group, dis_physical) %>%
  rename(Income = income_group)

plot_income_time <- ggplot(income_over_time, aes(x = year, y = dis_physical, color= Income, group = Income)) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)                      
  labs(
    title = "Physical Disability Over Time by Income Group",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_color_manual(values = colors) +
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(income_over_time$year), max(income_over_time$year), by = 2)  #every other year
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )

ggsave("interim_data/plots/dis_physical_by_income_time.png", plot = plot_income_time)
print(plot_income_time)
```

### Age

```{r age_graph_time}
load(file = "interim_data/clean_disability_data.RData")
showtext_auto()

colors <- viridis::viridis(4)
#colorRampPalette(c("#FFCCC2", "#cc3f14"))(4)

age_levels <- c("Under 50", "50-64", "65-80", "80 or Older")
individual_dis_age_4 <- individual_dis_age_4 %>%
  mutate(age_group_4 = factor(age_group_4, levels = age_levels, ordered = TRUE))

age_over_time<- individual_dis_age_4 %>%
  filter(race == "total", sex == "total", !is.na(age_group_4), var_type == "percent") %>%
  select(year, age_group_4, dis_physical) %>%
  rename(Age = age_group_4)

save(age_over_time, file = "interim_data/age_over_time_percent_disability.RData")

plot_age_time <- ggplot(age_over_time, aes(x = year, y = dis_physical, color= Age, group = Age)) +
  geom_line(size = 1.2) +                      
  geom_point(size = 2) + #points on the year (optional)                      
  labs(
    title = "Physical Disability Over Time by Age Group",
    x = NULL,
    y = "Percent of Population"
  ) +
  scale_color_manual(values = colors) +
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(age_over_time$year), max(age_over_time$year), by = 2),  #every other year
    ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )

ggsave("interim_data/plots/dis_physical_by_age_time.png", plot = plot_age_time)
print(plot_age_time)


```

```{r 65_above_ratio}
showtext_auto()
load(file = "interim_data/clean_disability_data.RData")

age_levels <- c("Under 50", "50-64", "65-80", "80 or Older")
individual_dis_age_4 <- individual_dis_age_4 %>%
  mutate(age_group_4 = factor(age_group_4, levels = age_levels, ordered = TRUE))

age_65_over_dis <- individual_dis_age_3 %>%
  filter(race =="total", sex == "total", var_type == "percent", age_group_3 == "65+") %>%
  select(year, age_group_3, dis_physical)

age_65_over_dis_any <- individual_dis_any_age_3 %>%
  filter(race =="total", sex == "total", var_type == "percent", age_group_3 == "65+") %>%
  select(year, age_group_3, dis_any)


load(file = "interim_data/over_65.RData")

age_65_over_total <- older_65 %>%
  filter(var_type == "percent", race =="total", sex == "total") %>%
  select(year, `65+`)
#   
# plot_65_over_dis <- ggplot(age_65_over_dis, aes(x = year, y = dis_physical)) +
#   geom_line(size = 1.2, color = "#FFCCC2") +                      
#   geom_point(size = 2, color = "#f60") + #points on the year (optional)                      
#   labs(
#     title = "Percent of Population Aged 65 or Older With Physical Disability",
#     x = NULL,
#     y = "Percent of Population"
#   ) +
#   theme_minimal(base_family = "Montserrat", base_size = 22) +
#   scale_x_continuous(
#     breaks = seq(min(age_65_over_dis$year), max(age_65_over_dis$year), by = 2),  #every other year
#     ) +
#   theme(
#     axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
#     plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
#     legend.position = "top"
#   )
# print(plot_65_over_dis)

plot_65_over_total <- ggplot(age_65_over_total, aes(x = year, y = age_65_over_total$`65+`)) +
  geom_line(size = 1.2, color = "#440154") +                      
  geom_point(size = 2, color = "#440154") +                    
  labs(
    title = "Percent of Population Aged 65 or older",
    x = NULL,
    y = "Percent of Population"
  ) +
  theme_minimal(base_family = "Montserrat", base_size = 22) +
  scale_x_continuous(
    breaks = seq(min(age_65_over_total$year), max(age_65_over_total$year), by = 2),  #every other year
    ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
    legend.position = "top"
  )
print(plot_65_over_total)



```

# Kentucky Housing Corporation (KHC)

Below is a graph showing the number of accessible units in various housing developments provided by KHC. The total number of housing units in the data set was 35,862, making the largest category of accessible units (all categories combined) about 3.59% of units. *Note: The differences in unit features based on categories ADA compliant, Physical Disabilities, and Special Needs have not yet been defined.*

```{r KHC}
# NOTE- this code was generated in part by an LLM converting the python code found in KHC_analysis.Rmd into R code. Results of this code have been cross checked with original (python) results to verify accuracy of code

library(readxl)
library(dplyr)
library(ggplot2)
showtext_auto()

KHC_df <- read_excel("data/KHC Project Listings by Unit Category.xlsx")

# Total units
total_units <- sum(KHC_df$Units, na.rm = TRUE)

# Summarize by category
summary_KHC_df <- tibble(
  Category = c("ADA Compliant", "Physical Disabilities", "Special Needs")
) %>%
  rowwise() %>%
  mutate(
    Units = case_when(
      Category == "ADA Compliant" ~
        sum(
          KHC_df$Units[
            grepl("ADA", KHC_df$`Unit Category`, ignore.case = TRUE)
          ],
          na.rm = TRUE
        ),

      Category == "Physical Disabilities" ~
        sum(
          KHC_df$Units[
            grepl("Physical", KHC_df$`Unit Type`, ignore.case = TRUE) &
              !grepl("ADA", KHC_df$`Unit Category`, ignore.case = TRUE)
          ],
          na.rm = TRUE
        ),

      Category == "Special Needs" ~
        sum(
          KHC_df$Units[
            grepl("Special", KHC_df$`Unit Category`, ignore.case = TRUE)
          ],
          na.rm = TRUE
        )
    ),
    Percent = Units / total_units * 100
  ) %>%
  ungroup()

# Plot
plot_summary <- ggplot(summary_KHC_df, aes(x = Category, y = Units)) +
  geom_col(fill = "#440154", width = 0.5) +
  geom_text(
    aes(label = Units),
    vjust = -0.4,
    size = 6,
    fontface = "bold"
  ) +
  labs(
    title = "KHC Accessible Units",
    subtitle = "Categories below are listed as defined in data provided by KHC",
    x = NULL,
    y = "Number of Units"
  ) +
  theme_minimal(base_family = "Montserrat", base_size = 22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1),
    plot.title = element_text(face = "bold", size = 26)
  )

print(plot_summary)
```

# Projections: Jefferson County

## Age

::: age-projections-graph-65
:::

::: age-projections-graph-80
:::

## Ambulatory Difficulty

::: amb-projections
:::

# American Housing Survey (AHS)
## Household Level Accessibility Estimates

For Jefferson County, the number of households that include at least one person who uses any mobility assistive device is estimated to be 43,517, and around 18,031 include at least one person with either a manual or powered wheelchair. 

```{r household_accessibility_devices}
library(showtext)
showtext_auto()
font_add_google("Montserrat", "Montserrat")

load(file = "interim_data/accessible_features_hh.RData")

# percentage of households that include at least one person that uses a mobility device (by device type)
hh_devices <- accessible_features %>%
  summarise(Any = weighted.mean(any_mob_dev, WEIGHT, na.rm = TRUE) *100, 
            Cane = weighted.mean(CANE, WEIGHT, na.rm = TRUE)*100, 
            `Power Wheelchair` = weighted.mean(ECHAIR, WEIGHT, na.rm = TRUE)*100, 
            `Manual Wheelchair` = weighted.mean(WCHAIR, WEIGHT, na.rm = TRUE)*100,
            `Other Device` = weighted.mean(MOBOTHER, WEIGHT, na.rm = TRUE)*100, 
            Crutches = weighted.mean(CRUTCH, WEIGHT, na.rm = TRUE)*100
            ) %>%
  pivot_longer(cols = Any:Crutches, names_to = "Device Type", values_to = "Percent") %>%
  mutate(`Device Type` = factor(`Device Type`,
    levels = c("Any", "Cane", "Power Wheelchair", "Manual Wheelchair", "Crutches", "Other Device")))

plot1 <- ggplot(hh_devices, aes(x = `Device Type`, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")), 
            vjust = -0.5, size = 6) +
  labs(
    title = "Percent of Households in Which Someone Uses a Mobility Assistive Device",
    x = NULL,
    y = "Percent of Population"
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
  )
print(plot1)

ggsave("interim_data/plots/hh_devices.png", plot= plot1)

```

## Accessing Areas of the Home {.tabset}

### Households Including a Person Who Uses a Mobility Assistive Device

```{r hh_difficulty}

load(file = "interim_data/accessible_features_hh.RData")

# For homes with someone who uses a mobility device, what percent report difficulty accessing parts of their home
hh_room_difficulty <- accessible_features %>%
  mutate(across(c(HAGETKIT, HAGETBED, HAGETBATH, HAGETHOME), 
    ~ case_when(.x == 1 ~ TRUE, .x %in% c(-6, -9) ~ NA, .x == 2 ~ FALSE, TRUE ~ NA)
  )) %>%
  filter(any_mob_dev) %>%
  mutate(any_hh_diff = HAGETKIT == 1 | HAGETBATH == 1| HAGETBED == 1 | HAGETHOME == 1) %>%
  summarise(Any = weighted.mean(any_hh_diff, WEIGHT, na.rm = TRUE)*100,
            Entry = weighted.mean(HAGETHOME, WEIGHT, na.rm = TRUE)*100, 
            Kitchen = weighted.mean(HAGETKIT, WEIGHT, na.rm = TRUE)*100, 
            Bathroom = weighted.mean(HAGETBATH, WEIGHT, na.rm = TRUE)*100, 
            Bedroom = weighted.mean(HAGETBED, WEIGHT, na.rm = TRUE)*100) %>%
    pivot_longer(cols = Any:Bedroom, names_to = "Area of House", values_to = "Percent")

plot2 <- ggplot(hh_room_difficulty, aes(x = `Area of House`, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")), 
            vjust = -0.5, size = 6) +
  labs(
    title = "Percent of Households Which Report Difficulty Accessing Parts of Their Home",
    subtitle = "For homes with at least one person who uses a mobility assistive device", 
    x = NULL,
    y = NULL
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
  )
print(plot2)

ggsave("interim_data/plots/hh_diff.png", plot= plot2)

```

### Households Including a Person Who Uses a Wheelchair

```{r}
hh_room_difficulty_wheelchair <- accessible_features %>%
  mutate(across(c(HAGETKIT, HAGETBED, HAGETBATH, HAGETHOME), 
    ~ case_when(.x == 1 ~ TRUE, .x %in% c(-6, -9) ~ NA, .x == 2 ~ FALSE, TRUE ~ NA)
  )) %>%
  filter(any_chair) %>%
  mutate(any_hh_diff = HAGETKIT == 1 | HAGETBATH == 1| HAGETBED == 1 | HAGETHOME == 1) %>%
  summarise(Any = weighted.mean(any_hh_diff, WEIGHT, na.rm = TRUE)*100,
            Entry = weighted.mean(HAGETHOME, WEIGHT, na.rm = TRUE)*100, 
            Kitchen = weighted.mean(HAGETKIT, WEIGHT, na.rm = TRUE)*100, 
            Bathroom = weighted.mean(HAGETBATH, WEIGHT, na.rm = TRUE)*100, 
            Bedroom = weighted.mean(HAGETBED, WEIGHT, na.rm = TRUE)*100) %>%
  pivot_longer(cols = Any:Bedroom, names_to = "Area of House", values_to = "Percent")

plot4 <- ggplot(hh_room_difficulty_wheelchair, aes(x = `Area of House`, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")), 
            vjust = -0.5, size = 6) +
  labs(
    title = "Percent of Households Which Report Difficulty Accessing Parts of Their Home",
    subtitle = "For homes with at least one person who uses a wheelchair", 
    x = NULL,
    y = "Percent of Population"
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
  )
print(plot4)

ggsave("interim_data/plots/hh_diff_wchair.png", plot= plot4)
```

### All Households

```{r}

# For ALL homes, what percent report difficulty accessing parts of their home
hh_room_difficulty_ALL <- accessible_features %>%
  mutate(across(c(HAGETKIT, HAGETBED, HAGETBATH, HAGETHOME), 
    ~ case_when(.x == 1 ~ TRUE, .x %in% c(-6, -9) ~ NA, .x == 2 ~ FALSE, TRUE ~ NA)
  )) %>%
  mutate(any_hh_diff = HAGETKIT == 1 | HAGETBATH == 1| HAGETBED == 1 | HAGETHOME == 1) %>%
  summarise(Any = weighted.mean(any_hh_diff, WEIGHT, na.rm = TRUE)*100,
            Entry = weighted.mean(HAGETHOME, WEIGHT, na.rm = TRUE)*100, 
            Kitchen = weighted.mean(HAGETKIT, WEIGHT, na.rm = TRUE)*100, 
            Bathroom = weighted.mean(HAGETBATH, WEIGHT, na.rm = TRUE)*100, 
            Bedroom = weighted.mean(HAGETBED, WEIGHT, na.rm = TRUE)*100) %>%
    pivot_longer(cols = Any:Bedroom, names_to = "Area of House", values_to = "Percent")

plot3 <- ggplot(hh_room_difficulty_ALL, aes(x = `Area of House`, y = Percent)) +
  geom_col(fill = "#440154", width = .5) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")), 
            vjust = -0.5, size = 6) +
  labs(
    title = "Percent of Households Which Report Difficulty Accessing Parts of Their Home",
    subtitle = "For all homes", 
    x = NULL,
    y = NULL
  ) +
 theme_minimal(base_family = "Montserrat",base_size =  22) +
  theme(
    axis.text.x = element_text(angle = 30, hjust= 1, family = "Montserrat"),
    plot.title = element_text(face = "bold", size = 26, family = "Montserrat"),
  )
print(plot3)

ggsave("interim_data/plots/hh_diff_ALL.png", plot= plot3)

```

```{r excel, eval=FALSE}
df_list <- list(
  Overall    = overall_excel,
  Sex        = sex_excel,
  Race       = race_excel,
  Age        = age_excel,
  Income     = income_excel,
  HH_Income  = hh_income_excel
)

new_dfs <- list(
  Overall = overall_excel, 
  Sex = full_sex_excel, 
  Race = full_race_excel, 
  Age = full_age_excel, 
  Income = full_income_excel, 
  HH_Income = full_hh_income_excel
)

write.xlsx(df_list, file = "data/accessibility_data.xlsx", overwrite = TRUE)
```