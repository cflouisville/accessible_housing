...
this_df <- individual_dis %>%
  filter(var_type == "percent", sex == "total", race == "total")


this_df <- individual_dis_age_9 %>%
  filter(var_type %in% c("percent", "CI"), sex == "total", race == "total")
...
``` {r}
#TODO- recode sex -> Sex and male, female to caps too (for ggplot aesthetics)
  dis_by_sex <- individual_dis %>%
  filter(year ==2023, var_type == "percent", sex %in% c("male", "female"), race == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_any),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(3,6,7) %>%
  ...
  scale_fill_manual(values = c("male" = "darkblue", "female" = "lightblue")) +  # <-- note the +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1),
    plot.title = element_text(face = "bold", size = 16)
