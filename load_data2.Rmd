...
dis_by_sex <- individual_dis %>%
  ...
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any, dis_sensory),
               names_to = "Disability_Type", 
               values_to = "Percent")%>%
  select(sex,Disability_Type,Percent) %>%
...
dis_by_race <- df %>%
  ...
  mutate(race = recode(race, 
                      ...
                      white = "White", 
                      other = "Not Listed"), 
         race = factor(race, levels = c("Black", "White", "Hispanic", "Not Listed"))) %>% 
...
dis_map <- left_join(glptools::map_nh, dis_phys_nh, by = "neighborhood")
...
race_over_time <- df %>%
  filter(race != "total", name == "dis_physical") %>%
  select(-name, year)%>%
  mutate(race = recode(race, 
                      black = "Black", 
                      hispanic = "Hispanic", 
...
hh_devices <- accessible_features %>%
  summarise(Any = weighted.mean(any_mob_dev, WEIGHT, na.rm = TRUE) *100, 
            ...
            `Powered Wheelchair` = weighted.mean(ECHAIR, WEIGHT, na.rm = TRUE)*100, 
            `Manual Wheelchair` = weighted.mean(WCHAIR, WEIGHT, na.rm = TRUE)*100,
            `Other Mobility Device` = weighted.mean(MOBOTHER, WEIGHT, na.rm = TRUE)*100, 
