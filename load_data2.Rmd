...
rm(individual_dis_physical, individual_dis_independent, individual_dis_selfcare, individual_dis_vision,
   individual_dis_hearing, individual_dis_cognitive, individual_dis_any)
...
df <- df %>%
  ...
  summarise(rate = sum(estimate / population * weight)) %>%
  ungroup()
...
income_all <- income_and_hearing %>%
  ...
  left_join(income_and_any, 
            by = c("FIPS","year","sex","race","var_type","income_group")) %>%
  left_join(income_and_cognitive, 
            by = c("FIPS","year","sex","race","var_type","income_group")) %>%
...
homeownership_all <- homeownership_dis_physical %>%
  ...
  left_join(homeownership_dis_cognitive,by = c("FIPS","year","sex","race","var_type","homeownership")) %>%
...
rm(homeownership_dis_physical, homeownership_dis_independent, 
   ...
   homeownership_dis_cognitive, homeownership_dis_hearing, homeownership_dis_any)
...
hh_homeownership_all <- hh_homeownership_dis_physical %>%
  ...
  left_join(hh_homeownership_dis_cognitive,by = c("FIPS","year","sex","race","var_type","homeownership")) %>%
...
hh_income_all <- hh_income_dis_physical %>%
  ...
  left_join(hh_income_dis_cognitive, by = c("FIPS","year","sex","race","var_type","income_group")) %>%
...
age_all <- age_and_hearing %>%
  ...
  left_join(age_and_cognitive,  by = c("FIPS","year","sex","race","var_type", group_var)) %>%
...
#factorize so legend is ordered
age_levels <- c("50-54", "55-59","60-64", "65-69", "70-74", "75-79", "80-84", "85-90", "90 plus")

age_all_5 <- age_all_5 %>%
  mutate(age_group_5_yrs = factor(age_group_5_yrs, levels = age_levels, ordered = TRUE))
...
dis_by_race <- individual_dis %>%
  filter(year == 2024, var_type == "percent", race != "total", sex == "total") %>%
  pivot_longer(cols = c(dis_physical, dis_independent, dis_selfcare, dis_vision, dis_hearing, dis_cognitive, dis_any),
               names_to = "Disability_Type", 
               values_to = "Percent") %>%
  select(4,6,7) %>%
...
### Ambulatory Disability and Race (by Age)
```{r}
#can change to wider disability group > physical, self care, vision, hearing

colors <- colorRampPalette(c("#FFCCC2", "#cc3f14"))(4)

disability_data_fake <- disability_data %>%
  filter(year %in% 2020:2024) %>%
  mutate(year = 2024)


disability_data %>% filter(year == 2024, race=="hispanic", age_group_4 == "80 or Older") %>% View()

individual_dis_age_4     <- survey_by_demog(disability_data_fake, 
                                        "dis_physical", 
                                        "PERWT", 
                                        other_grouping_vars = "age_group_4")

dis_race_age <- individual_dis_age_4 %>%
  filter(year == 2024, sex == "total", !is.na(age_group_4), race != "total") %>%
  mutate(race = recode(race, 
                      white = "White", black = "Black", hispanic = "Hispanic", other= "Not Listed" )) %>% 
  rename(Age = age_group_4, Race = race, Percent = dis_physical)

#var_type == "percent", 

plot_race_age <- ggplot(dis_race_age, aes(x = Race, y = Percent, fill = Age)) +  # fill says split x category by this category
  geom_col(position = position_dodge(width = 0.5), width = 0.5) + #dodge = side by side not stacked 
  labs(
...
showtext_auto()
colors <- colorRampPalette(c("#FFCCC2", "#cc3f14"))(9)
...
geom_segment(
 ...
 theme_minimal(base_family = "Montserrat", base_size = 22) +
  ...
  scale_color_manual(values = c("Any" = all_color, "Ind. Living" = colors[1], "Self-Care" = colors[2], "Hearing" = colors[3], "Physical" = colors[4] , "Cognitive" = colors[5], "Vision" = colors[6])) +
  geom_text_repel(
    data = last_points,
    aes(label = Label),
    nudge_x = 1.5, # push labels into white space
    hjust = 0,
    direction = "y",
    family = "Montserrat",
    fontface = "bold",
...
dis_prev_by_type_over_time2 <- dis_prev_by_type_over_time %>%
  filter(Disability_Type == "Any")
