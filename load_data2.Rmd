...
income_levels <- c("150k+", "100k–149.9k", "75k–99.9k","50k–74.9k", "20k–49.9k", "Under 20k")
...
plot6 <- ggplot(dis_by_own, aes(x = Disability_Type, y = Percent, fill = Homeownership)) +  # fill says split x category by this category
  ...
  scale_fill_manual(values = c("Owns" = "lightblue", "Rents" = "darkblue")) +  
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1),
    plot.title = element_text(face = "bold", size = 16)
...
plot4 <- ggplot(dis_by_income, aes(x = Disability_Type, y = Percent, fill = `Income Group`)) +  # fill says split x category by this category
  ...
  scale_fill_manual(values = c("150k+"    = "#c6dbef",
                                "100k–149.9k"    = "#9ecae1",
                                "75k–99.9k"    = "#6baed6",
                                "50k–74.9k"    = "#4292c6", 
                                "20k–49.9k"  = "#2171b5",
...
```{r age_chart}
# @H im guessing were going to need to tweak the age groups again bc over 80 group is just so much higher... but also maybe thats just the way it will be no matter how we slice her up 
...
plot5 <- ggplot(dis_by_age, aes(x = Disability_Type, y = Percent, fill = Age)) +  # fill says split x category by this category
  ...
  scale_fill_manual(values = c("50-64" = "darkblue", 
                               "65-80" = "lightblue", 
                               "80 or Older"= "lightgrey" , 
                               "Under 50"= "orange")) +  
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1),
...
## Disability Trends Over Time (2008-2023)
...
sex_over_time<- individual_dis %>%
  filter(year %in% c(2008, 2010, 2012, 2014, 2016, 2018, 2021, 2023), sex %in% c("male", "female"), var_type == "percent") %>% 
...
#graph
plot_sex_time <- ggplot(sex_over_time, aes(x = factor(year), y = dis_physical, fill = Sex)) +  
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +  # side-by-side bars
  labs(
...
plot_race_time <- ggplot(race_over_time, aes(x = factor(year), y = dis_physical, fill = Race)) +  
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +  # side-by-side bars
  labs(
...
plot_owns_time <- ggplot(homeownership_over_time, aes(x = factor(year), y = dis_physical, fill = Homeownership)) +  
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +  # side-by-side bars
  labs(
...
plot_income_time <- ggplot(income_over_time, aes(x = factor(year), y = dis_physical, fill = Income)) + 
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +  # side-by-side bars
  labs(
...
plot_age_time <- ggplot(age_over_time, aes(x = factor(year), y = dis_physical, fill = Age)) +
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +  # side-by-side bars
  labs(
...
# Kentucky Housing Corporation (KHC) Data
